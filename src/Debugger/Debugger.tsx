...
import { Grid, GridItem, Tab, TabContent, Tabs, TabTitleText } from '@patternfly/react-core';
  ...
  return (
    <Grid hasGutter style={{ overflowY: 'hidden' }}>
      ...
      <GridItem style={{ overflowY: 'auto' }}>
        ...
        <TabContent eventKey={2} id="entitlements-tab" hidden={activeTabKey !== 2} style={{ display: activeTabKey === 2 ? 'flex' : 'none', height: '100%' }}>
          <Entitlements user={user} />
        </TabContent>
        <TabContent eventKey={3} id="roles-tab" hidden={activeTabKey !== 3} style={{ display: activeTabKey === 3 ? 'flex' : 'none', height: '100%' }}>
          <Roles user={user} />
        </TabContent>
      </GridItem>
    </Grid>
