...
export const WithPayloads: Story = {
  parameters: {
    flags: [
      { name: 'feature.complex-config', enabled: true, variant: { name: 'variant-a', payload: { theme: 'dark', maxItems: 50, features: ['search', 'filter'] } } },
      { name: 'feature.simple-string', enabled: true, variant: { name: 'beta', payload: 'beta-v2.1.0' } },
      { name: 'feature.numeric', enabled: true, variant: { name: 'limit', payload: 100 } },
      { name: 'feature.no-payload', enabled: true, variant: { name: 'enabled' } },
    ],
  },
};
...
export const ManyFlags: Story = {
  parameters: {
    flags: [
      ...defaultFlags,
      { name: 'platform.patch.new-ui', enabled: true, variant: { name: 'enabled' } },
      { name: 'platform.vulnerability.scanning', enabled: false, variant: { name: 'disabled' } },
      { name: 'platform.compliance.reports', enabled: true, variant: { name: 'enabled' } },
      { name: 'platform.drift.comparison', enabled: false, variant: { name: 'disabled' } },
      { name: 'platform.ros.recommendations', enabled: true, variant: { name: 'enabled' } },
      { name: 'platform.edge.fleet-management', enabled: true, variant: { name: 'enabled' } },
    ],
  },
};
...
export const ToggleOverrideTest: Story = {
  play: async ({ canvasElement }) => {
    ...
    const darkModeSwitch = canvas.getByLabelText('Toggle platform.chrome.dark-mode');
...
export const ClearOverridesTest: Story = {
  play: async ({ canvasElement }) => {
    ...
    // Toggle two flags
    await userEvent.click(canvas.getByLabelText('Toggle platform.chrome.dark-mode'));
    await userEvent.click(canvas.getByLabelText('Toggle platform.insights.inventory-groups'));
